...
`vs record start <PAGE> [--fps N] [--retina]` records the page to an AV1 video while the agent works. Frames are downscaled to 1440px wide by default (a retina screen recording is heavy to capture and encode); `--retina` keeps full device resolution; `vs record stop <PAGE>` (alias `rec`) flushes and prints the file path (`~/.vibesurfer/captures/rec-<PAGE>.ivf`). Frames are captured in the background at `--fps` (1..=30, default 4) through the same path as `vs watch`, encoded with pure-Rust rav1e into an IVF container that ffmpeg, VLC, and any dav1d player read. One recording per page. MCP: `vs_record_start` (`{page, fps?}`) and `vs_record_stop` (`{page}`), both returning `path\t<file>`. Convert to mp4 or gif externally if you need those.
