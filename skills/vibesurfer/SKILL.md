...
version: 0.0.1
binary: vs
description: Agent-native headless browser. 19 primitives over a Unix-socket wire protocol. Real WKWebView (macOS), WebKitGTK (Linux), or WebView2 (Windows). Optimistic concurrency via state tokens; tree-delta wire format; durable session/page/auth state in SQLite.
---
...
| Backend | Renders | Viewport | Layout | Auth | Notes |
|---------|---------|----------|--------|------|-------|
| `webkit` (macOS) | ✅ | ✅ | ✅ | ✅ | System WebKit.framework, `WKWebView`. |
| `wpe` (Linux) | ✅ | partial | ✅ | ✅ | WebKitGTK 6 via `webkit6` crate. Needs `libwebkitgtk-6.0`. |
| `webview2` (Windows) | ✅ | — | — | ✅ | Microsoft Edge / Chromium via `webview2-com`. Skeleton — `open`/`snapshot` work; others TODO. |
| `stub` (all) | placeholder | placeholder | placeholder | placeholder | In-process, no FFI. Used only by tests; never shipped on the production path. |
