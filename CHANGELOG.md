...
### Changed (0.2)
- `vs record` is much lighter and its output is downscaled to 1280px wide by default (`--retina` / MCP `retina:true` keeps full device resolution). Three fixes stop recording from being a memory hog: the screen-capture path rendered a full uncompressed retina bitmap per frame via `TIFFRepresentation` that AppKit never freed (~100 MB/s leak) — now it renders to a CGImage once; the AV1 encoder buffered its whole lookahead window of uncompressed frames — now low-latency, one frame at a time; and frames stream straight to the IVF file. A recording that grew without bound now plateaus around 250 MB.
