...
### Added
- The daemon reaps what agents abandon. `vs serve` is auto-spawned and long-lived, and its callers are agents: one that crashes, times out or is killed never reaches `vs_session_close`, which is the ordinary way an agent run ends. Nothing ever brought the count back down. On the author's machine that reached 48 sessions and 162 open pages over four days — 173 WebKit processes, around 8.6 GB, a load average of 40 on ten cores, and a laptop nobody could work on; the only remedy was killing the daemon, which took live authenticated pages with it. Two stages now, because they cost different things. A page untouched for 30 minutes drops its web view and goes dormant: the row keeps its url, tree and token, and the next call rebuilds the view exactly as a session resurrected from SQLite does, so the agent sees a re-baselined view and nothing else. A session untouched for 24 hours is closed outright. Both windows are env-tunable (`VS_PAGE_IDLE_SECS`, `VS_SESSION_TTL_SECS`, `VS_REAP_SWEEP_SECS`) and `VS_REAP=0` restores the old behaviour. Cell `cell_idle_pages_are_reaped_and_recreated`. (Reported by mild-dove-37c0 via `#vibesurfer`.)
- `vs session-close --all` and `--idle-for=<2h|90m|30s>` sweep a workspace instead of one session, and `vs status` reports what a workspace actually costs: `sessions=N pages=M live=K` overall, plus `live=` and `idle=` per session and per page. `sessions=48` read as an ordinary number for four days; `live=162` would not have. Cell `cell_session_close_sweeps_idle_sessions`.
- `vs act` takes a mark where it takes a ref: `vs act <PAGE> mark:NAME <OP>`, and `vs_act` over MCP takes a `mark` property. Acting on a mark returned `NotImplemented` on every real backend, so a mark could be taken and listed but never used, and the agent still had to carry the ref — the thing a mark exists to stop it carrying. Marks are a store concept scoped to a session, so the daemon resolves one to a live ref before the engine is called and `ActTarget::Mark` is gone from the engine entirely. The recorded ref is a hint, checked by role: refs are sticky, and a label is what an element says rather than what it is, so a button that relabels itself every 250ms must not break its own mark. When the recorded ref no longer holds, the tree is searched for the captured role and label, and the answer comes back with `? mark_reaimed ref=N` — a mark that moved is when an agent most wants to look before it clicks. An unknown name is `! NOT_FOUND mark=NAME`, and a mark addressed through the wrong page is refused rather than silently re-pointed. Fixture `mark-reaim.html`, cell `cell_act_on_a_mark`.
- `vs auth save` carries IndexedDB, and `vs auth load` restores it. The blob held cookies plus local and session storage, so a site that keeps its session in IndexedDB — a local-first store, a Firebase or Supabase client — restored as logged out from a blob that reported success, which is the worst shape for this to fail in: the next call acted as though it had a session. None of the three engines can await a promise inside one `evaluateJavaScript`, so both directions are kickoff-and-poll scripts driven to completion by the host, with each backend handing in the same run-loop pump it already gives `run_wait`. Without that pump the polls spin through in microseconds while the database's callbacks, which need turns on that same thread, never run. A record JSON cannot carry (a `Blob`, a `File`, a typed array) is counted and reported as `? storage_partial indexeddb_records=N` instead of restored as an empty object. Restoring into a database that already exists at the captured version reopens one version up, since an object store can only be created in an upgrade transaction. Cell `cell_auth_carries_indexeddb`, verified on macOS.

### Fixed
- `vs inspect <PAGE> eval` no longer crashes the daemon's dispatch task on a non-ASCII expression. `redact_string`, which scrubs casually-pasted credentials out of the audit log, walked the expression byte by byte and sliced at each index, so the first multi-byte character — an em-dash in a comment, an accented string literal, any CJK text — panicked mid-codepoint and came back to the agent as `! ENGINE_CRASH` on a perfectly valid call. Characters that did not panic were rebuilt byte-as-char, which is mojibake for everything above ASCII. It now walks char boundaries. The two `vs_inspect request` body truncations that sliced at a fixed byte offset now route through `truncate_on_boundary`, next to which they sat, and whose comment already claimed every such truncation went through it.
- A `vs prompt-form` entry link shows its own fields and no others. The page rendered the whole pending queue, and a form's entries deliberately outlive the waiter that enqueued them, so an agent whose `vs_prompt_form_wait` ran out of budget and asked again left the first form pending and the human opened the new link to the same two fields twice, then three times, with nothing on the page saying which pair the agent was actually waiting on. The nonce minted by `vs_prompt_form` is now bound to that form: the GET renders only its entries and the POST fulfills only its ids. `vs pending url` is unscoped as before and still shows everything queued. Cell `cell_prompt_form_link_is_scoped_to_its_form`.
- A superseded `vs prompt-form` is cancelled instead of lingering. A form outlives the waiter that enqueued it (by design — an MCP host's tool-call cap routinely kills the first wait while the human is still typing), so an agent that asked again left the first one queued for its 30-minute orphan TTL, where nobody could ever collect it: the only caller who knew its id had moved on. It still showed on `vs pending list` and on the unscoped `vs pending url` page, which is the same duplicate-fields symptom through the other door. An identical ask — same page, same refs, in order — now cancels the older form, so the waiter still parked on it is told `was cancelled` rather than waiting out the TTL. A form with any field already fulfilled is never superseded: those values are the human's typing, and `restore` exists precisely so they are handed back rather than destroyed.
- The browser entry form has a Cancel button. Closing the tab told nobody anything: the agent stayed parked until its own budget ran out, unable to tell a human who had walked away from one still typing, and the entries sat in the queue until the orphan TTL. Cancel ends the form's entries at once, so `vs_prompt_form_wait` returns `was cancelled` — an answer the agent can act on. It carries `formnovalidate`, since the browser's own "please fill this in" would otherwise block the one button whose point is that the human does not want to fill it in, and it is scoped like the rest of the link: a form link cancels its own form and nothing else. Cell `cell_prompt_form_cancel_from_the_browser`.
...
### Changed
- The macOS daemon no longer burns CPU while idle. The Cocoa main thread drove the engine by waking every 4ms to poll an mpsc queue — an mpsc send is not a run-loop source and cannot wake a sleeping loop, so idle CPU and job-pickup latency were traded against each other. Workers now signal a real `CFRunLoopSource` when they queue a job, so the main thread sleeps until there is work. Idle CPU drops from ~1.5% to 0.00%, and because the poll's average 2ms queue wait is gone, every operation also got faster rather than this being a trade: `view` 10.5ms → 7.5ms, `read` 8.3ms → 6.6ms, `goto` 15.5ms → 13.4ms, `capture` 54ms → 50ms, `open` 97ms → 87ms (release build, median wall clock, ~6.5ms of which is CLI process start). Linux and Windows have the same polling shape and are untouched; the `Waker` hook on `EngineRuntime` is platform-agnostic so both can adopt it the same way.
...
- **Windows engine-tests were never actually passing.** `vs serve` died with `STATUS_STACK_OVERFLOW` (0xC00000FD) at startup on Windows: the platform reserves 1 MiB for a process's main thread against 8 on Unix, and `serve` does its whole startup there (store open, WebView2 backend, engine runtime, daemon), which a debug build does not fit into. The daemon crashed before binding its named pipe, so all 71 m6 cells failed at the test harness's 10s spawn deadline — and 71 × 10s pushed the job past its own 12-minute cap, killing it before the failure summary could print. The crash was invisible and `docs/REALITY_CHECK.md` claimed `yes` for every Windows cell on the strength of it. The binary now links with an 8 MiB stack reserve, the harness inlines `daemon.log` on spawn failure, and the job cap is 25 minutes. Windows is now genuinely green: 71/71.
- `vs wait` no longer fails on a merely-slow page. `run_wait` handed its 150ms poll interval to the predicate eval as a hard deadline, so one slow poll returned `! TIMEOUT 150ms eval` and aborted the whole wait. Poll pacing and per-eval budget are now separate, and a poll that times out is retried rather than fatal — only the wait's own budget ends the loop. This was the long-standing flakiness in `cell_wait_gone`, which had been written off as CI noise rather than the product bug it was.
...
- `vs type` now drains the run loop repeatedly after the last keystroke so the web-content process finishes delivering the `keyUp`, `beforeinput`, and `input` events before the call returns. A single drain was not enough under load, so a fast follow-up read (or a `vs view`) could miss the last character or two. The regression cell also polls for the text rather than reading once.
...
### Changed
- Resurrected pages are dormant until first use: daemon startup no longer reopens every page's webview up front (15 pages made the daemon unreachable long enough to look dead), and zombie sessions never cost a webview. The engine page is created on the first call that touches the page.

### Fixed
- `is_listening` on Unix now actually connects instead of checking that the socket file exists. A killed daemon leaves its socket file behind; clients took that as "daemon up", skipped auto-spawn, and every call failed with "connection refused" — which reads exactly like lost sessions. Auto-spawn's socket wait also grew 2s → 10s.
