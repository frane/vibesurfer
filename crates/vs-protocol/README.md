...
Keyboard input has the same trust story on all three engines: `vs type <TEXT>` sends real per-character key events into the focused element (`isTrusted = true`, full keydown → beforeinput → input pipeline), so rich-text editors like DraftJS and ProseMirror — which ignore the programmatic `act fill` path — accept it. Native `NSEvent` on macOS, XTest on Linux (or the RemoteDesktop portal under Wayland), and the DevTools protocol on Windows, where WebView2's visual hosting has no keyboard injection API of its own. The same fixture cell holds all three to an `isTrusted`-only mirror.
